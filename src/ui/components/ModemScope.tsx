  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      >
        <span>
          PLL:{' '}
          <span style={{ color: pilotFreq > 0 ? '#34d399' : '#f87171' }}>
            {pilotFreq > 0 ? `${pilotFreq.toFixed(1)}Hz` : 'unlocked'}
          </span>
        </span>
        <span>
          Amp: <span style={{ color: '#e5e7eb' }}>{pilotAmp.toExponential(2)}</span>
        </span>
        <span>
          Frame:{' '}
          <span style={{ color: inFrame ? '#34d399' : '#6b7280' }}>
            {inFrame ? 'DATA' : 'scanning'}
          </span>
        </span>
        <span>
          Trace: <span style={{ color: '#e5e7eb' }}>{trace.length} frames</span>
        </span>
      </div>
