    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ flex: 1, height: 12, background: "rgba(255,255,255,0.05)", borderRadius: 3, overflow: "hidden" }}>
          <div style={{
            width: `${vuPct}%`, height: "100%",
            background: vuDb > -12 ? "#f87171" : vuDb > -24 ? "#f59e0b" : vuDb > -40 ? "#34d399" : "#818cf8",
            borderRadius: 3, transition: "width 60ms linear",
          }} />
        </div>
        <span style={{ fontSize: 10, fontFamily: "monospace", color: "#e5e7eb", minWidth: 45, textAlign: "right" }}>
          {vuDb.toFixed(0)} dB
        </span>
      </div>
      ...
      <div style={{ display: "flex", gap: 16, fontSize: 10, color: "#6b7280" }}>
        <span>Peak: <span style={{ color: "#e5e7eb", fontFamily: "monospace" }}>{rawPeak.toExponential(2)}</span></span>
        <span>Noise: <span style={{ color: "#e5e7eb", fontFamily: "monospace" }}>{noiseFloorDb.toFixed(0)} dB</span></span>
      </div>

      {/* FFT Bars */}
      <canvas ref={fftRef} width={400} height={80} style={{
        width: "100%", height: 80, borderRadius: 4,
        background: "rgba(0,0,0,0.4)",
      }} />

      {/* Waterfall */}
      <canvas ref={waterfallRef} width={400} height={100} style={{
