...
export function BitAnalyzer({ debug, historyMax = 8 }: Props) {
  ...
  return (
    <div style={{ fontFamily: "monospace", fontSize: 10 }}>
      ...
      <div style={{
        display: "grid", gridTemplateColumns: "40px 30px 30px 60px 1fr",
        gap: "1px", background: "#1e1e3a", borderRadius: 4, overflow: "hidden",
        marginBottom: 4,
      }}>
      ...
      {bitTable.map(row => {
        const ePct = Math.min(100, (row.energy / Math.max(...(debug?.energies ?? [1]), 1e-12)) * 100);
        return (
          <div key={row.tone} style={{
            display: "grid", gridTemplateColumns: "40px 30px 30px 60px 1fr",
            gap: "1px", background: "#1e1e3a", marginBottom: 2, borderRadius: 3, overflow: "hidden",
          }}>
            ...
            <div style={{ ...tdStyle, color: row.flip ? "#ff6b4a" : "#6c6cff", fontWeight: row.flip ? 700 : 400 }}>
              {row.phaseBit}
            </div>
            <div style={{
              ...tdStyle,
      ...
      <div style={{
        marginTop: 6, padding: "4px 6px", background: "#07070e", borderRadius: 3,
        border: "1px solid #1e1e3a", fontSize: 9, color: "#5858a0",
      }}>
        Frame: <span style={{ color: "#e0e0ee", letterSpacing: "0.05em" }}>
...
const thStyle: React.CSSProperties = {
  background: "#11111e", padding: "3px 4px",
  fontSize: 8, color: "#5858a0", textTransform: "uppercase", letterSpacing: "0.04em",
  fontWeight: 600,
};
