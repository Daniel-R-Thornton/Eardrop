...
export const FloatingWindow: React.FC<FloatingWindowProps> = ({
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => setSize({
      width: Math.max(200, resizeRef.current.startSize.width + (e.clientX - resizeRef.current.startX)),
      height: Math.max(150, resizeRef.current.startSize.height + (e.clientY - resizeRef.current.startY)),
    });
  ...
  const panel = (
    <div style={{
      ...
      background: '#0e0e18', border: '1px solid #2a2a3e', borderRadius: 10,
      ...
      <div onMouseDown={handleMouseDown} style={{
        ...
        padding: '4px 10px', background: '#15152a', borderBottom: '1px solid #2a2a3e',
        cursor: 'grab', flexShrink: 0, fontWeight: 600, color: '#6c6cff',
      }}>
        <span>{title}</span>
        <div className="fw-header-btns" style={{ display: 'flex', gap: 4 }}>
          {onClose && (
            <button onClick={onClose} style={{
              background: 'none', border: '1px solid #3a3a5e', color: '#8888a0',
              cursor: 'pointer', padding: '0 6px', borderRadius: 4, fontSize: 12, lineHeight: '18px',
            }}>✕</button>
          )}
        </div>
      </div>
      ...
      <div onMouseDown={handleResizeStart} style={{
        ...
        cursor: 'nwse-resize', background: 'linear-gradient(135deg, transparent 50%, #3a3a5e 50%)',
      }} />
    </div>
  );
