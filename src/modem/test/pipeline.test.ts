...
describe('Modem Pipeline', () => {
  ...
  describe('Best Case — Clean Loopback', () => {
    it(
      'should pass with 64 bytes payload',
      async () => {
    ...
    it(
      'should pass with 256 bytes payload',
      async () => {
    ...
    it(
      'should discover pilot frequency accurately',
      async () => {
        const result = await harness.runCleanTest({ payloadBytes: 64 });
  ...
  describe('AWGN SNR Sweep', () => {
    ...
    it(
      'should at least partially decode at 10 dB SNR',
      async () => {
        const result = await harness.runSimulatedTest(
          'SNR 10dB',
          { snrDb: 10 },
  ...
  describe('Doppler Shift Tolerance', () => {
    it(
      'should pass with +2 Hz Doppler shift',
      async () => {
        const result = await harness.runSimulatedTest(
          'Doppler +2Hz',
          { dopplerHz: 2 },
    ...
    it(
      'should pass with -1 Hz Doppler shift',
      async () => {
        const result = await harness.runSimulatedTest(
          'Doppler -1Hz',
          { dopplerHz: -1 },
  ...
  describe('Multi-path Echo Tolerance', () => {
    it(
      'should pass with mild echo (16 samples, 30%)',
      async () => {
        const result = await harness.runSimulatedTest(
          'Echo mild',
  ...
  describe('Phase Noise Tolerance', () => {
    it(
      'should pass with low phase noise (0.05 rad)',
      async () => {
        const result = await harness.runSimulatedTest(
          'Phase noise 0.05',
          { phaseNoiseStd: 0.05 },
  ...
  describe('Amplitude Modulation Tolerance', () => {
    it(
      'should pass with slow volume drift',
      async () => {
        const result = await harness.runSimulatedTest(
          'Amp mod drift',
  ...
  describe('Stress Test — All Impairments Combined', () => {
    it(
      'should at least sync and partially decode under full stress',
      async () => {
  ...
  describe('Throughput Measurement', () => {
    it(
      'should measure clean loopback throughput',
      async () => {
        const result = await harness.runCleanTest({ payloadBytes: 256 });
    ...
    it(
      'should measure throughput at 20 dB SNR',
      async () => {
        const result = await harness.runSimulatedTest(
          'Throughput @20dB',
          { snrDb: 20 },
        ...
        const bitsPerSec = bytesPerSec * 8;
        console.log(
          `📊 Throughput (20dB SNR): ${bytesPerSec.toFixed(1)} byte/s = ${bitsPerSec.toFixed(0)} bit/s`,
        );
      },
      TEST_TIMEOUT,
    );
  ...
  describe('Worst Case — Maximum Impairments', () => {
    it(
      'should still function at lowest usable SNR',
      async () => {
  ...
  describe('SNR → BER Curve', () => {
    it(
      'should produce decreasing BER with increasing SNR',
      async () => {
        const results = await harness.runSnrSweep([5, 10, 15, 20, 25, 30], { payloadBytes: 128 });
        ...
        for (let i = 1; i < results.length; i++) {
          if (results[i - 1].metrics.rawBer > 0 && results[i].metrics.rawBer > 0) {
            expect(results[i].metrics.rawBer).toBeLessThanOrEqual(
              results[i - 1].metrics.rawBer * 1.5,
            );
          }
        }
      },
  ...
  describe('Payload Size Scaling', () => {
    it(
      'should handle small payload (16 bytes)',
      async () => {
    ...
    it(
      'should handle medium payload (512 bytes)',
      async () => {
    ...
    it(
      'should handle large payload (2048 bytes)',
      async () => {
