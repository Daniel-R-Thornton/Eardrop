...
export class BerTracker {
  ...
  recordEccResult(totalBits: number, rawErrors: number, corrected: number, uncorrectable: number): void {
...
export class ConstellationSampler {
  ...
  record(tone: number, i: number, q: number, ampBit: number, phaseBit: number,
         trueAmpBit?: number, truePhaseBit?: number): void {
  ...
  recordFrame(
    ...
    for (let t = 0; t < 4; t++) {
      this.record(
        t, relI[t], relQ[t],
        ampBits[t], phaseBits[t],
        trueAmpBits?.[t], truePhaseBits?.[t],
      );
    }
    this.symbolCounter++;
  }
