...
import { DebugLogger, LogEvent, STAGE, LOG_LEVEL, StageTag } from "./debugger";
import { BerReport } from "./diag";
...
export function compressForLLM(
  ...
  // BER report
  if (berReport) {
    output.push(`[BER] raw=${berReport.rawBer.toExponential(2)} corr=${berReport.correctedBer.toExponential(2)} total_bits=${berReport.totalBits} err_raw=${berReport.rawErrors} err_corr=${berReport.correctedErrors} err_uncorr=${berReport.uncorrectableErrors}`);
  }
