...
export class TxEngine {
  ...
  /** Pilot phase accumulator */
  private pilotPhase = new PhaseAcc();
  /** Per-tone phase accumulators */
  private tonePhases: PhaseAcc[];
  ...
  constructor(cfg: Partial<ModemConfig> = {}) {
    ...
    this.tonePhases = [new PhaseAcc(), new PhaseAcc(), new PhaseAcc(), new PhaseAcc()];
  }

  reset(): void {
    this.pilotPhase.reset();
    for (const p of this.tonePhases) p.reset();
  }
  ...
  transmitFrame(header: AtomicHeader, payload: Uint8Array): Float32Array {
    ...
    for (let sym = 0; sym < totalSymbols; sym++) {
      // Read 4 bits for this symbol
      const bpskMul: number[] = [];
      for (let t = 0; t < TONE_COUNT; t++) {
        const bit = bitIdx < bits.length ? bits[bitIdx++] : 0;
        bpskMul.push(bit === 0 ? 1 : -1);
      }
