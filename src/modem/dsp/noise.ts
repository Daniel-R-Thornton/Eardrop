...
export class AGC {
  ...
  process(samples: number[]): number[] {
    ...
    let desiredGain =
      this.smoothedRms > 1e-12 ? this.cfg.targetRms / this.smoothedRms : this.currentGain;
...
export class PilotAGC {
  ...
  update(measuredPilotAmplitude: number): void {
    const desired =
      measuredPilotAmplitude > 1e-12 ? this.targetAmp / measuredPilotAmplitude : this.gain;
 ...
 */
export function logNoiseStats(subtractor: SpectralSubtractor, agc: AGC, pilotAgc: PilotAGC): void {
  debugLogger.info(
    STAGE.DEBUG,
    {
      noise_floor_avg:
        subtractor['noiseFloor']?.reduce((a: number, b: number) => a + b, 0) /
