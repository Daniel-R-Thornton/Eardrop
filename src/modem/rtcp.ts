...
export class ChannelProfiler {
  ...
  private computeResponse(): FrequencyResponse {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const chirpPhase = 2 * Math.PI * (
        this.sweepStartFreq * t +
        sweepWidth * t * t / (2 * this.sweepDuration)
      );
    ...
    for (let b = 0; b < numBands; b++) {
      ...
      const mag = expectedMag[b] > 1e-12
        ? Math.min(2, measured / expectedMag[b])
        : 0;
      magnitude[b] = mag;
      ...
      snr[b] = noiseFloor > 1e-12 && magnitude[b] > 1e-12
        ? 20 * Math.log10(magnitude[b] / noiseFloor)
        : 0;
    }
