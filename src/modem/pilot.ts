...
export class PilotScanner {
  ...
  feedSample(_sample: number): PilotDiscovery | null {
    ...
    const top5 = results.slice(0, 5).map(r => `${r.f}Hz=${r.m.toExponential(2)}`).join(', ');
    const median = results.map(r => r.m).sort((a, b) => a - b)[Math.floor(results.length / 2)] || 1e-12;
  ...
  reset() {
    this.rtBuf = []; this.done = false; this.result = null; this.scanCount = 0;
    this.noiseBuf = []; this.noiseLearned = false;
  }
}
...
export class PilotPLL {
  ...
  update(sample: number): void {
    const error = sample * this.sinRef, prop = this.cfg.Kp * error;
...
export function getDataToneFreqs(pilotFreqHz: number, musical = false): [number, number, number, number] {
  const offs = musical ? [300, 425, 550, 775] : [TONE_OFFSETS[0], TONE_OFFSETS[1], TONE_OFFSETS[2], TONE_OFFSETS[3]];
  return [
