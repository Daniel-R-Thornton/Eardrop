...
export class TestHarness {
  ...
  async runStressTest(options?: TestOptions): Promise<TestResult> {
    ...
    return this.runTest('Full Stress', opts, {
      ...
      snrDb: 10,
      dopplerHz: 3,
      echoes: [{ delaySamples: 16, attenuation: 0.3 }, { delaySamples: 32, attenuation: 0.1 }],
  ...
  private async runTest(
    ...
    const configData = (() => {
      ...
      buf.set(configPayload, off); off += configPayload.length;
    ...
    try {
      ...
      return {
        ...
        metrics: {
          dataMatch: false, byteErrors: testData.length, totalBytes: testData.length,
          rawBer: 1, correctedBer: 1, blocksDecoded: 0, blocksCrcFailed: 0,
          syncAcquired: false, syncTimeMs: 0, pilotDiscovered: false, pilotFreq: 0,
        },
        fileComplete: false,
        squawkCount: 0,
        squawkAvgDrift: 0,
    ...
    const result: TestResult = {
      ...
      squawkAvgDrift: sqHistory.length > 0
