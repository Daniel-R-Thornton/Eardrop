...
export function generatePreamble(cfg: PreambleConfig): Float32Array {
  ...
  // Duration in samples
  const warbleSamps = Math.floor(0.4 * sampleRate); // 1280
  const calSamps = Math.floor(0.16 * sampleRate); // 512
  const invSamps = Math.floor(0.16 * sampleRate); // 512
  const sweepSamps = Math.floor(0.2 * sampleRate); // 640
  ...
  const warbleInterval = Math.floor(0.01 * sampleRate); // 32 samples
