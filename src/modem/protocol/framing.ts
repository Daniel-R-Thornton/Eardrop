...
export const BLOCK_OVERHEAD =
  SENTINEL_BYTES + BLOCK_HEADER_BYTES - SENTINEL_BYTES + BLOCK_FOOTER_BYTES; // 5
...
export function encodeBlock(type: BlockType, data: Uint8Array, sentinel?: number): EncodedBlock {
  ...
  buf[off++] = crc & 0xff;
  // eslint-disable-next-line no-useless-assignment -- off tracks byte position through buffer pack
  buf[off++] = (crc >> 8) & 0xff;
...
export function decodeBlock(
  bytes: Uint8Array,
  sentinel?: number,
): { type: BlockType; data: Uint8Array } | null {
  const s = sentinel ?? SENTINEL;
    ...
    switch (this.phase) {
      case 'SCAN':
        if (this.bitCount >= 24 && this.shiftReg === this.sentinel) {
          this.phase = 'HEADER';
          console.warn(
            `[FRAME] sentinel @ bit ${this.totalBits} sr=0x${this.shiftReg.toString(16)} sentinel=0x${this.sentinel.toString(16)}`,
          );
          this.byteAccum = 0;
          this.byteBits = 0;
          this.bytesCollected = [];
          ...
          if (this.expectedBytes <= 0) {
            ...
            if (crc16(crcInput) === crc) {
              ...
              if (this.blocksCrcFailed <= 3) {
                ...
                console.warn(
                  `[FRAME] CRC fail: type=0x${pb.type.toString(16)} len=${pb.len} got=0x${crc.toString(16)} exp=0x${expCrc.toString(16)} @ bit ${this.totalBits}`,
                );
              }
            }
